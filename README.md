...
**Seeded accounts:** every first-run account gets a **strong random password
printed exactly once** to the terminal/server log (there are no built-in
default passwords — ever). To choose your own instead, set it before seeding:

```bash
export SEED_ADMIN='A-Very-Long-Passphrase-You-Choose!'
python run.py seed
```
...
| MD/CEO | `md` |
| Admin Manager | `am.funke`, `am.emeka` |
| HODs | `hod.medicine`, `hod.surgery`, `hod.paeds`, `hod.emergency`, `hod.pharmacy`, `hod.lab` |

All seeded accounts must change their password at first login
(`must_change_password`), and every `SEED_<USERNAME>` variable follows the same
naming rule (dots become underscores, e.g. `SEED_HOD_MEDICINE`).
