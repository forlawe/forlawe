...
def recurring_findings(history: Iterable[dict], window: int = 10, threshold: int = 3) -> List[str]:
    ...
    optionally criteria_version: int (F-069).
    ...
    Only inspections recorded under the CURRENT criteria wording are compared:
    criterion numbers changed meaning between versions, so counting a v1 record
    against a v2 record as the "same" problem would be a false claim.
    """
    current = CRITERIA_VERSION
    recent = [h for h in list(history)[:window]
              if int(h.get("criteria_version") or current) == current]
    if len(recent) < 3:
        return []
    messages = []
    for no in range(1, CRITERIA_COUNT + 1):
