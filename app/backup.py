    ...
    try:
        with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
            with db.engine.connect().execution_options(
                ...
                for table in tables:
                    # Identifiers are ALWAYS quoted. On PostgreSQL an unquoted
                    # `FROM user` is parsed as the SQL function user() — it
                    # returns the role name, NOT the table — so the archive's
                    # user.csv silently contained one column ("user", the role)
                    # and every real staff row was missing from PG backups.
                    # SQLite never hit this (it has no bare-`user` function),
                    # which is why only the PostgreSQL restore drill caught it.
                    q = f'"{table}"'
                    ...
                    if table == "stored_file":
                        stmt = _text(
                            f"SELECT id, key, org_id, folder, filename, "
                            f"content_type, size, sha256 FROM {q}")  # noqa: S608
                    else:
                        stmt = _text(f"SELECT * FROM {q}")  # noqa: S608
