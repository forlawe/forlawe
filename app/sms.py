...
def send_sms(msg: SmsMessage, already_claimed: bool = False) -> SmsMessage:
    ...
    F-064: process_sms_queue claims rows atomically first and passes
    already_claimed=True; direct single sends keep the old behaviour.
    """
...
def process_sms_queue(limit: int = 30) -> int:
    """Send everything queued (or stuck mid-send). Returns count processed.

    F-064: rows are CLAIMED ATOMICALLY (UPDATE ... WHERE status='QUEUED' /
    stuck-SENDING), so concurrent workers can never both send the same SMS.
    """
    ...
    def _reclaimable():
        ...
        return db.and_(
            ...
            db.or_(SmsMessage.claimed_at.is_(None),
                   SmsMessage.claimed_at < cutoff))

    cutoff = now_naive() - timedelta(minutes=2)
    msgs = (db.session.query(SmsMessage)
            .filter(SmsMessage.attempts < 3,
                    db.or_(SmsMessage.status == "QUEUED", _reclaimable()))
            .order_by(SmsMessage.created_at).limit(limit).all())
    processed = 0
    for m in msgs:
        res = db.session.execute(
            sa_update(SmsMessage)
            .where(SmsMessage.id == m.id,
                   SmsMessage.attempts < 3,
                   db.or_(SmsMessage.status == "QUEUED", _reclaimable()))
            .values(status="SENDING",
                    claimed_at=now_naive(),
                    attempts=SmsMessage.attempts + 1))
        db.session.commit()
        if res.rowcount != 1:
            continue                      # another worker won this row — no send
        # commit() expired the object; attributes (incl. the attempts the claim
        # just incremented) reload from the database on next access.
        send_sms(m, already_claimed=True)
        processed += 1
    return processed
