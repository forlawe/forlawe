...
from .models import (AppNotification, Complaint, CorrectiveAction, DutyRoster,
                     LoginAttempt, Organization, db, now_naive)
...
def job_purge_login_attempts(app):
    """F-063 — stop the failed-login black book growing forever.

    LoginAttempt rows are only useful while they hold an ACTIVE lockout or
    recently-failed history. Rows that never recorded a failure carry no
    information at all, and rows idle for 180 days are ancient history — both
    are deleted (locked rows are always kept until their lockout expires).
    Bounded, so one pass never churns the whole table.
    """
    from .models import LoginAttempt as _LA
    now = now_naive()
    idle_cutoff = now - timedelta(days=180)
    purged = 0
    for _ in range(200):
        rows = (db.session.query(_LA)
                .filter(db.or_(_LA.locked_until.is_(None), _LA.locked_until < now),
                        db.or_(_LA.last_failure_at.is_(None),
                               _LA.last_failure_at < idle_cutoff))
                .limit(50).all())
        if not rows:
            break
        for row in rows:
        ...
        db.session.commit()
    if purged:
        app.logger.info("login-attempts: purged %d stale row(s)", purged)
...
JOB_SEQUENCE = (job_duty_reminders, job_overdue_inspection, job_complaint_sla,
                ...
                job_retention_purge, job_purge_login_attempts)
