...
# The bootstrap accounts created on first run. The usernames are stable so an
# operator can pre-set passwords through SEED_<USERNAME> environment variables,
# but the passwords themselves are NEVER hardcoded anywhere: seed_data refuses
# to create an account with anything but an explicit override or a fresh
# cryptographically-random password (F-074).
SEED_USERNAMES = (
...
def _generated_password() -> str:
    """Strong random password: ~71 bits from token_urlsafe(12) plus a fixed
    complexity tail so it satisfies any naive policy checker too."""
    return secrets.token_urlsafe(12) + "A1!"


def _resolve_passwords(overrides: dict | None) -> dict:
    """One password per seed account.

    Caller-supplied overrides win; any account WITHOUT an override gets a fresh
    random password (never a known string). The caller can read the returned
    dict to print the credentials exactly once.
    """
    overrides = overrides or {}
    missing = [u for u in SEED_USERNAMES if u not in overrides]
    if missing:
        print(f"[seed] No password override for: {', '.join(missing)} — "
              "generating strong random passwords (printed below).")
    return {u: (overrides[u] if u in overrides else _generated_password())
...
def seed_data(app, demo: bool = False, passwords: dict | None = None,
    ...
    with app.app_context():
        if db.session.query(Organization).first():
            return None
        creds = _resolve_passwords(passwords)
        ...
        if announce:
            ...
            print("-" * 70)
            for uname, pw in creds.items():
                print(f"  {uname:14s} / {pw}")
            print("=" * 70)
...
def auto_seed(app):
    ...
    for uname in SEED_USERNAMES:
        ...
        if val:
            overrides[uname] = val
    seed_data(app, passwords=overrides or None,
