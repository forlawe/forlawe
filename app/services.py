...
def recurring_flags_for_department(org_id: int, department_id: int) -> list[str]:
    ...
    # F-069: trend logic must only compare inspections recorded under the SAME
    # criteria wording. A criterion number in v1 meant something different than
    # in v2, so mixing versions in a "recurring problem" count is a false claim.
    hist = [i for i in hist
            if (i.criteria_version or scoring.CRITERIA_VERSION) == scoring.CRITERIA_VERSION]
    as_dicts = [{"criterion_scores": inspection_criterion_map(i),
                 "criteria_version": i.criteria_version or scoring.CRITERIA_VERSION}
                for i in hist]
