...
def main() -> int:
    ...
    with app.app_context():
        ...
        for model, fields in (
            ...
            # This job legitimately rewrites PHI for EVERY hospital, so it
            # must declare cross-hospital scope on PostgreSQL. RLS otherwise
            # hands it an empty result set and the command reports a cheerful
            # "0/0 rows rewritten" while real plaintext PHI stays plaintext —
            # exactly the failure a silent 0 hides.
            rls.all_orgs()
            ...
            for row in rows:
                ...
                if done and done % 200 == 0:
                    db.session.commit()
                    rls.all_orgs()   # commit dropped the transaction-local GUC
            db.session.commit()
