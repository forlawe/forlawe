...
def rate_limit(limit: int = 10, window: float = 60.0, key_extra: str = "",
    ...
    Constitution Rule 5: the key must be the actual distinguishable actor,
    never blindly the source IP — many legitimate users share one IP behind
    mobile carriers, USSD gateways and corporate NATs. Pass `key_func` (e.g.
    a function returning the caller's phone number from the payload) when the
    actor is identifiable from the request itself; the source IP remains the
    fallback when no actor can be found.
    """
    def deco(fn):
        @functools.wraps(fn)
        def wrapper(*a, **kw):
            ...
            actor = key_func() if key_func is not None else client_ip()
            key = f"{request.endpoint}|{actor}|{key_extra}"
