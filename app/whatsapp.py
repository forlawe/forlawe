...
def send_message(msg: WhatsAppMessage, already_claimed: bool = False) -> WhatsAppMessage:
    ...
    F-064: when called from process_queue the row was ALREADY claimed
    atomically (status=SENDING, attempts+1, claimed_at stamped) so
    already_claimed=True skips re-incrementing — a second worker can never
    win the same row. Direct single retries (admin retry button) pass
    already_claimed=False and keep the old behaviour.
    """
    cfg = current_app.config
...
def process_queue(limit: int = 20) -> int:
    ...
    F-064: every row is CLAIMED ATOMICALLY before sending:
        UPDATE ... SET status='SENDING', attempts=attempts+1, claimed_at=now
        WHERE id=... AND status='QUEUED'
    Only ONE worker can win a given row (the conditional UPDATE locks the row
    and re-checks the status), so two web processes or two background threads
    can never both send the same message — exactly-once dispatch under
    concurrency, not best-effort.

    Stuck-SENDING recovery (expert review 2026-08-27): a worker that died
    mid-send leaves status=SENDING. Those rows are reclaimed only when their
    claimed_at is older than 2 minutes (legacy rows with no claimed_at fall
    back to created_at), so a message that is being sent RIGHT NOW is never
    double-claimed by a second worker.
    """
    ...
    def _reclaimable():
        ...
        return db.and_(
            ...
            db.or_(WhatsAppMessage.claimed_at.is_(None),
    ...
    msgs = (db.session.query(WhatsAppMessage)
            .filter(
                ...
                db.or_(WhatsAppMessage.status == "QUEUED", _reclaimable())
            )
            .order_by(WhatsAppMessage.created_at).limit(limit).all())
    processed = 0
    for m in msgs:
        # Atomic claim — the row itself decides who wins.
        was_stuck = m.status == "SENDING"
        res = db.session.execute(
            sa_update(WhatsAppMessage)
            .where(WhatsAppMessage.id == m.id,
                   WhatsAppMessage.attempts < 3,
                   db.or_(WhatsAppMessage.status == "QUEUED", _reclaimable()))
        ...
        db.session.commit()
        if res.rowcount != 1:
            # Another worker claimed it first — send NOTHING.
            continue
        if was_stuck:
            current_app.logger.warning(
                "whatsapp: reclaiming stuck SENDING id=%s", m.id)
        # commit() expired the object; attributes (incl. the attempts the claim
        # just incremented) reload from the database on next access.
        send_message(m, already_claimed=True)
        processed += 1
    return processed
