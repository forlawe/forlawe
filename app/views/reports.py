...
def department_report(dept_id: int):
    ...
    # F-069: criterion averages only over records scored under the CURRENT
    # wording — criterion numbers changed meaning across versions (v2, 18 Aug
    # 2026), so mixing versions into one average is a false claim.
    hist_cur = [i for i in hist
                if (i.criteria_version or scoring.CRITERIA_VERSION) == scoring.CRITERIA_VERSION]
    criterion_avgs = {}
    for no in range(1, 6):
        vals = [s.score for i in hist_cur for s in i.scores if s.criterion_no == no]
...
def escalations_report():
    ...
    items = (db.session.query(Complaint)
             ...
             .order_by(Complaint.escalated_at.desc()).limit(500).all())
