...
# F-062: a hash to verify against when the submitted username does not exist.
# scrypt is deliberately slow, so skipping the hash for unknown users made the
# login endpoint a username oracle — attackers could measure the faster "no
# such user" response. We always pay the hashing cost, against this dummy hash,
# so unknown and known usernames answer in indistinguishable time.
_DUMMY_PASSWORD_HASH = generate_password_hash(
    "timing-equaliser-dummy-password", method="scrypt")
...
def login_post():
    ...
    if user is None:
        # F-062: burn the same scrypt time as a real account check so response
        # timing cannot reveal which usernames exist on this server.
