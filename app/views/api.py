...
# ================================================================ USSD / gateway intake
def _ussd_actor() -> str:
    """F-077 — key USSD rate limits to the real caller, never blindly to IP.

    Every USSD request arrives through the telecom gateway's shared IP, so an
    IP-keyed limiter makes the whole hospital share one tiny bucket. Prefer the
    session id (one per USSD session), then the caller's phone number, and only
    fall back to IP when neither exists.
    """
    from ..security import client_ip as _cip
    try:
        data = request.get_json(silent=True) or {}
        form = request.form.to_dict() if request.form else {}
        merged = {**data, **form}
        for key in ("sessionId", "session_id", "phoneNumber", "phone", "msisdn"):
...
@bp.post("/ussd/queue")
@rate_limit(limit=1200, window=60.0, key_extra="ussd_ip")
@rate_limit(limit=20, window=60.0, key_extra="ussd_caller", key_func=_ussd_actor)
def ussd_queue():
...
@rate_limit(limit=6000, window=60.0, key_extra="ussd_ip")
@rate_limit(limit=60, window=60.0, key_extra="ussd_session", key_func=_ussd_actor)
def ussd_callback():
    ...
    # Choice 3: Check Status
        ...
        if len(remaining) >= 2:
            ...
            try:
                from ..models import QueueTicket
                # USSD audit (F-049): ticket codes recycle EVERY day (G-001 is
                # today's first ticket AND last month's), so a bare code proves
                # nothing. Only the caller's OWN phone number may resolve a
                # ticket — today's or older alike. A sequential code must never
                # reveal a stranger's queue status.
                callers = sorted({p for p in (phone_norm, phone_raw) if p})
                t = None
                if callers:
                    t = (db.session.query(QueueTicket)
                         .filter(QueueTicket.org_id == org.id,
                                 ...
                                 QueueTicket.phone.in_(callers)).first())
                    if t is None:
                        t = (db.session.query(QueueTicket)
                             .filter(QueueTicket.org_id == org.id,
                                     QueueTicket.code == code_input,
                                     QueueTicket.phone.in_(callers))
                             .order_by(QueueTicket.id.desc()).first())
                if not t:
                    return Response(
                        "END Ticket not found. Check the code and dial from "
                        "the phone number used to join.",
                        mimetype="text/plain")
                # Estimate position
                try:
                    from .. import queue_estimator
