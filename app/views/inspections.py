...
def inspection_detail(insp_id: int):
    ...
    return render_template("inspection_detail.html", insp=insp, scores=scores_by_no,
                           cas=cas, trend=trend,
                           criteria=scoring.criteria_for(insp.criteria_version),  # F-065: show wording this record was scored under
                           can_amend=bool(can_amend), users=users)
...
def inspection_amend(insp_id: int):
    ...
    old = Inspection(
        ...
        criteria_version=insp.criteria_version,  # F-065: a signed record keeps the wording it was scored under
