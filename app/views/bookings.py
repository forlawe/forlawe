...
def portal_submit():
    ...
    if not apt_created:
        # same browser re-submitted an identical booking — still show summary
        session["thanks_booking_ref"] = apt.ref
    ...
    db.session.commit()
    # F-066: the thank-you page may only show this booking's details to the
    # browser that JUST created it — a bare sequential ref must not reveal
    # a stranger's department/date/time to anyone who guesses or reuses a URL.
    session["thanks_booking_ref"] = apt.ref
    return redirect(url_for("bookings.portal_thanks", ref=apt.ref))
...
def portal_thanks():
    ...
    # F-066: `ref` is sequential and guessable (ORG-APT-2026-000042), so it is
    # NOT a secret. Show booking details only when this browser just created
    # that booking (session breadcrumb set by the create route); anyone else —
    # including someone re-opening a shared or old link — gets the generic
    # confirmation with the reference only, never the department/date/time.
    apt = None
    if ref and session.pop("thanks_booking_ref", None) == ref:
