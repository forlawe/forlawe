...
def roster_view():
    ...
    return render_template(
        ...
        # F-070: the Reassign control may only appear for the two roles the
        # roster_reassign endpoint itself authorizes — a button that 403s for
        # the person who can see it is an orphaned-route trap, not a link.
        can_reassign=current_user.role in ("SUPER_ADMIN", "HEAD_ADMIN_HR"),
...
def leave_approve(rid: int):
    ...
    if req.status != "PENDING":
        ...
        return redirect(url_for("roster.leave_list"))
    # F-047: duty and leave are mutually exclusive, checked in BOTH directions.
    # Bulk import already refuses to put someone on duty when they are on
    # leave; approving leave must equally refuse when the person is already
    # rostered for DUTY inside the requested window — otherwise the roster
    # silently shows the same person on duty AND on leave.
    duty_rows = (db.session.query(RosterEntry)
                 .filter(RosterEntry.org_id == org_id,
                         RosterEntry.user_id == req.user_id,
                         RosterEntry.kind == "DUTY",
                         RosterEntry.duty_date >= req.start_date,
                         RosterEntry.duty_date <= req.end_date)
                 .order_by(RosterEntry.duty_date).all())
    if duty_rows:
        dates = ", ".join(r.duty_date.strftime("%d %b %Y") for r in duty_rows)
        flash(f"{req.user.name} is already rostered for duty on: {dates}. "
              "Remove that duty entry first, then approve the leave.", "error")
        return redirect(url_for("roster.leave_list"))
    # Check balance
