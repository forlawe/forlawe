...
def create_app(config_object=None, scheduler: bool = True) -> Flask:
    ...
    for blueprint in (auth_bp, mfa_bp, main_bp, insp_bp, comp_bp, book_bp, queue_bp, fb_bp,
                      ...
                      lahsma_bp,
                      admin_bp, rolesadmin_bp, deptdesk_bp, tv_bp,
    ...
    with app.app_context():
        ...
        if db_ready:
            ...
            def _seed_roles():
                ...
                # Boot seeding is a cross-hospital job: with row-level security
                # already FORCEd on (later boots of an existing database),
                # inserts into role/branch are blocked unless the connection
                # declares all-orgs scope — rls.all_orgs() is exactly what its
                # own docstring names "the boot seeder" for. Without it, role
                # and branch seeding silently fail on every PostgreSQL boot
                # after the first, and AUTO_SEED bootstrap breaks in CI/tests
                # that run as a non-superuser (superusers bypass RLS, which is
                # why the SQLite-only and superuser runs never saw this).
                try:
                    from . import rls
                    rls.all_orgs()
                except Exception:                       # noqa: BLE001
                    pass    # SQLite: no-op; keep boot resilient
