...
def find_cross_branch_departments(org_id: int) -> list[str]:
    """Integrity check: flag departments whose activity spans >1 branch (F-056, option a)."""
    from .models import Department, QueueEntry, Visit, DutyShift
    dept_ids = [d[0] for d in db.session.query(Department.id).filter_by(org_id=org_id, active=True).all()]
    flagged = []
    for dept_id in dept_ids:
        branches = set()
        for model in (QueueEntry, Visit, DutyShift):
            rows = db.session.query(model.branch_id).filter_by(department_id=dept_id, org_id=org_id).filter(model.branch_id.isnot(None)).all()
            branches.update(r[0] for r in rows)
        if len(branches) > 1:
            dept = db.session.get(Department, dept_id)
            dept_name = dept.name if dept else f"id={dept_id}"
            flagged.append(f"{dept_name}: spans branches {sorted(branches)}")
    return flagged
