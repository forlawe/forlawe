...
from .scoring import CRITERIA_VERSION  # single source of truth for the criteria wording (F-065)
...
class Inspection(db.Model):
    ...
    # F-065/F-069: every inspection is stamped with the criteria wording that
    # applied when it was recorded, drawn from the single named constant at
    # write time — never inferred later from a timestamp. Reports/trends must
    # never mix scores recorded under different criteria meanings.
    criteria_version = db.Column(db.Integer, nullable=False, default=lambda: CRITERIA_VERSION)
...
class SmsMessage(db.Model):
    ...
    status = db.Column(db.String(12), default="QUEUED", index=True)  # QUEUED|SENDING|SENT|FAILED
    provider_id = db.Column(db.String(80))
    attempts = db.Column(db.Integer, default=0)
    last_error = db.Column(db.String(400))
    created_at = db.Column(db.DateTime, default=now_naive)
    # F-064: atomic-claim timestamp — see WhatsAppMessage.claimed_at.
...
class WhatsAppMessage(db.Model):
    ...
    last_error = db.Column(db.String(400))
    created_at = db.Column(db.DateTime, default=now_naive)
    # F-064: when a worker atomically CLAIMS a row it stamps the time here; the
    # stuck-SENDING reaper only touches rows whose claim is older than the
    # timeout, so an in-flight message is never double-sent by a second worker.
    claimed_at = db.Column(db.DateTime, nullable=True, index=True)
    sent_at = db.Column(db.DateTime)
...
class Patient(db.Model):
    ...
    # F-015 / PG parity: ciphertext needs ~4x the plaintext. The declared
    # length is the STORED (encrypted) length; 32 held plaintext only and
    # PostgreSQL rejects the longer Fernet token with StringDataRightTruncation
    # (SQLite never checks VARCHAR length, hiding the bug). k27 widens to 256.
...
class ReceptionIntake(db.Model):
    ...
    nok_name = db.Column(db.String(120))
    # F-015 / PG parity: stored length is the ENCRYPTED length (256, matching
    # migration k27). 32 could hold only plaintext; Fernet tokens overflow on
    # PostgreSQL (StringDataRightTruncation) while SQLite silently accepts.
