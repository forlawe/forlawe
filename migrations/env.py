...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        ...
        # CRITICAL (found on PostgreSQL, invisible on SQLite): the advisory-lock
        # SELECT above auto-began a transaction on this connection. If it is
        # left open, alembic's context.begin_transaction() nests inside a
        # SAVEPOINT, every migration "runs" and reports success, and the outer
        # transaction is ROLLED BACK when the connection closes — the database
        # is never actually migrated. Close the lock's transaction first; the
        # advisory lock itself is session-level and survives the commit.
        if connection.dialect.name == "postgresql":
            connection.commit()
        try:
