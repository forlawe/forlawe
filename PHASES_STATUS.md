...
## Final verification addendum (2026-09-09, Lagos)

Phase 7 and Phase 8 acceptance work completed on top of the phase history above:

**Phase 7 visual/UX polish (guarded by tests/test_f080_contrast_tokens.py):**
- F-080: status/neutral colour tokens darkened to WCAG AA (>=4.5:1) and pinned by a
  test that computes real contrast from the actual token values in app.css.
- F-082/F-083/F-084: icon nav links carry aria-labels; nav buttons and the back
  link are real 44px touch targets styled from app.css (no inline template CSS).
- F-085: dashboard heatmap colours come from shared CSS tokens, never inline hex.
- F-086: sales page and TOTP issuer consistently brand the product
  ("HospitalSuite"), matching the rest of the app; the stray "CareQueue" name is gone.

**Phase 8 CI/ops hardening:**
- F-053: the full pentest.run() checklist is a build gate (any `fail` level fails
  the suite). F-009: coverage is measured by tooling in CI, never typed by hand.
- CI definition lives in .github/workflows/tests.yml (SQLite + coverage, PostgreSQL
  as a NON-SUPERUSER role so RLS tests are meaningful, empty-database migration
  check, dependency audit). Note: enabling it needs a push with a workflow-scope
  GitHub token (the GitHub App token used for this branch cannot push workflow files).

**Final PostgreSQL verification pass — real product bugs found and fixed** (each
invisible on SQLite, which is why the suite must run on both engines; the PG empty-
DB migration path and the RLS tests cannot be simulated with SQLite):
- migrations/env.py: the advisory-lock SELECT left an open transaction, so on
  PostgreSQL every migration ran inside a savepoint and was ROLLED BACK on close —
  alembic reported success while the database was never migrated.
- k25/k26: boolean columns added with `DEFAULT 0` / updated with `SET col=0` —
  PostgreSQL rejects both (DatatypeMismatch); the swallowed errors aborted the
  migration transaction.
- k28: guess-and-drop constraints inside try/except — every missed drop aborted
  the PG transaction and constraint creation was silently skipped.
- m30/RLS: `whatsapp_message` is not the real table name (`whats_app_message` is),
  so migrated DBs never got claimed_at and RLS never protected the WhatsApp table.
- app/backup.py: unquoted `FROM user` on PostgreSQL reads the SQL user() function,
  so backups silently contained zero staff rows.
- app/encrypt_phi_backfill.py: never declared cross-org scope, so under RLS it
  reported "0/0 rows rewritten" while plaintext PHI stayed plaintext.
- app/models.py: nok_phone declared EncryptedString(32) but Fernet ciphertext needs
  256 — PostgreSQL enforces VARCHAR length, SQLite does not.
