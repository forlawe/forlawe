...
# ------------------------------------------------------------------ F-013 position
def _intake(org, stage, created_at, n):
    """Minimal valid ReceptionIntake row (ref unique, names required).

    The row must reference a REAL patient: PostgreSQL enforces the
    patient_id foreign key, while SQLite has FK enforcement off by default —
    a literal patient_id that points at no patient passes on SQLite and
    fails on PostgreSQL with IntegrityError. Mirror _visit(): create and
    flush the Patient first, then use its actual id.
    """
    from app.models import Patient
    p = Patient(org_id=org, hospital_number=f"ESTI/{n:05d}",
                surname=f"Intake{n}", first_name="Pat", sex="F",
                age_years=30, payer_type="SELF", category="GENERAL")
    db.session.add(p)
    db.session.flush()
    return ReceptionIntake(
        org_id=org, patient_id=p.id, stage=stage, ref=f"EST/{n:05d}",
...
def test_position_in_stage_counts_only_people_ahead(app, seeded):
    ...
    with app.app_context():
        org = seeded["org"]
        early = _intake(org, "RECEPTION", now_naive() - timedelta(minutes=20), 1)
        late = _intake(org, "RECEPTION", now_naive(), 2)
...
def test_position_in_stage_falls_back_to_open_count_when_patient_unknown(app, seeded):
    with app.app_context():
        org = seeded["org"]
        db.session.add(_intake(org, "BILLING", now_naive(), 3))
        db.session.commit()
...
def test_journey_estimates_differ_by_real_position(app, seeded):
    ...
    with app.app_context():
        org = seeded["org"]
        now = now_naive()
        early = _intake(org, "RECEPTION", now - timedelta(minutes=30), 4)
        late = _intake(org, "RECEPTION", now, 5)
        db.session.add_all([early, late])
        db.session.commit()
