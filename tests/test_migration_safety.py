...
# --------------------------------------------------------------------------
# F-064 / RLS follow-ups: names must match the REAL schema, verified by the
# same empty-DB upgrade path CI runs — never by create_all (create_all builds
# the truth from the models, so it cannot catch a migration that targets a
# table name that does not exist). Two real bugs lived here: m30 added
# claimed_at to "whatsapp_message" (the table is "whats_app_message"), and
# rls.PROTECTED_TABLES protected the same non-existent name — so on
# PostgreSQL the WhatsApp table silently had no claim column AND no RLS.
# --------------------------------------------------------------------------
def test_every_rls_protected_table_name_is_a_real_table(app):
    """RLS must never name a table that does not exist: enable() skips it
    silently and the real table stays unprotected forever."""
    from app.rls import PROTECTED_TABLES
    real = set(db.metadata.tables)
    missing = [t for t in PROTECTED_TABLES if t not in real]
    assert not missing, (
        f"PROTECTED_TABLES names a table that is not a model: {missing}. "
        "RLS enable() skips missing tables silently — a typo here leaves "
        "real tenant data unprotected on PostgreSQL.")


def test_empty_database_upgrade_to_head_has_claim_columns_and_indexes(tmp_path):
    """The CI migration check (`alembic upgrade head` on an EMPTY database)
    must deliver m30's columns AND indexes on the real message tables."""
    import tempfile
    from alembic import command
    from alembic.config import Config
    ...
    for table in ("whats_app_message", "sms_message"):
        ...
        assert "claimed_at" in cols, (
            f"empty-DB upgrade to head left {table} without claimed_at — the "
            "migration named the wrong table or never ran on this one")
        idx = {r[1] for r in con.execute(f"PRAGMA index_list({table})")}
        assert f"ix_{table}_claimed_at" in idx, (
            f"{table}.claimed_at has no index after migration (models say "
            "index=True) — prod schema would drift from dev")
    con.close()
    os.unlink(tmp.name)


# --------------------------------------------------------------------------
# PostgreSQL empty-DB upgrade guard. SQLite tolerates `DEFAULT 0` booleans,
# non-batched constraint drops and uncommitted advisory-lock transactions, so
# none of the PG-only migration failures above can ever be seen by the SQLite
# leg. This test RUNS ONLY when TEST_DATABASE_URL points at PostgreSQL (the
# CI PostgreSQL job sets it; locally it skips) and proves the whole chain
# applies to a brand-new database and persists — exactly what a fresh
# Supabase/Render database does on first deploy.
# --------------------------------------------------------------------------
def test_postgres_empty_database_upgrades_to_head_and_persists():
    import os as _os
    url = (_os.environ.get("TEST_DATABASE_URL")
           or _os.environ.get("DATABASE_URL") or "")
    if not url.startswith("postgresql"):
        import pytest as _pytest
        _pytest.skip("PostgreSQL-only guard: point DATABASE_URL / "
                     "TEST_DATABASE_URL at a postgres:// URL to run it")
    ...
    base = url.rsplit("/", 1)[0]
    admin = base + "/postgres"
    scratch = base + "/hms_empty_upgrade_guard"
    con = psycopg2.connect(admin)
    con.autocommit = True
    cur = con.cursor()
    cur.execute("DROP DATABASE IF EXISTS hms_empty_upgrade_guard")
    # OWNER omitted: the connecting role (the CI PostgreSQL superuser) becomes
    # the owner by default.
    cur.execute("CREATE DATABASE hms_empty_upgrade_guard")
    con.close()
    ...
    try:
        ...
        con = psycopg2.connect(scratch)
        con.autocommit = True
        cur = con.cursor()

    cur.execute("SELECT version_num FROM alembic_version")
    assert cur.fetchone()[0] == "m30_message_claim", \
        "empty PostgreSQL database did not reach m30_message_claim"
    cur.execute("SELECT count(*) FROM information_schema.tables "
                "WHERE table_schema='public'")
    n = cur.fetchone()[0]
    assert n >= 50, f"migrations did not persist: only {n} tables after upgrade"
    # the three PG-only fixes, present after a real chain run
    for table in ("whats_app_message", "sms_message"):
        cur.execute("SELECT 1 FROM information_schema.columns WHERE "
                    "table_name=%s AND column_name='claimed_at'", (table,))
        assert cur.fetchone(), f"{table}.claimed_at missing after PG upgrade"
    cur.execute("SELECT 1 FROM pg_constraint WHERE conrelid='user'::regclass "
                "AND conname='uq_user_org_username'")
    assert cur.fetchone(), "uq_user_org_username missing after PG upgrade"
    con.close()
    con = psycopg2.connect(admin)
    con.autocommit = True
    cur = con.cursor()
