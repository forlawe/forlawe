...
def _raw_column(db, table, col, org_id):
    with db.engine.connect() as conn:
        # Under PostgreSQL, tests that boot a second app (e.g. the backfill's
        # own create_app) leave RLS enabled on the test tables. A fresh,
        # unscoped connection then correctly sees NOTHING (unset tenant =
        # see nothing), which would make these assertions pass vacuously on an
        # empty result. Reading "what is stored at rest" is a legitimate
        # cross-hospital read — scope this one connection like app/backup.py.
        if conn.dialect.name == "postgresql":
            conn.execute(db.text("SELECT set_config('app.current_org', '-1', false)"))
        rows = conn.execute(
...
def test_backfill_command_encrypts_legacy_rows(app, seeded, monkeypatch):
    ...
    with app.app_context():
        ...
        # bf.main() booted a second app, which enabled RLS on the test
        # tables. Any background read must then declare cross-hospital scope
        # (see app/encrypt_phi_backfill) or RLS hands back an empty result.
        from app import rls
        rls.all_orgs()
