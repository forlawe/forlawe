...
def test_auto_seed_bootstraps_empty_db_once(app, monkeypatch):
    ...
    with app2.app_context():
        # Boot enabled row-level security on the protected tables (user is one
        # of them). Reading the seeded users afterwards is a legitimate
        # cross-hospital read — declare all-orgs scope like any boot job.
        rls.all_orgs()
