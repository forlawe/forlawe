...
from app.models import (Organization, Patient, PatientVisit, VisitOnward,
                        db, now_naive)
...
def _ensure_org():
    """PostgreSQL enforces foreign keys (SQLite does not). Every row below
    carries org_id=1, so org 1 must really exist or every insert in this file
    dies on IntegrityError under PostgreSQL. Create it once per test (each
    test gets a fresh database from the app fixture)."""
    if db.session.get(Organization, 1) is None:
        db.session.add(Organization(id=1, code="FTG",
                                    name="Fast Track Guard Hospital"))
        db.session.flush()
    return 1
